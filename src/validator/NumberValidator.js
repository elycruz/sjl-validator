 ...
 * @todo update algorithms in this module since their logic is from an older base
 *  and may have gaps in some places (namely how we validate numbers with
 *  mix-and-matched attributes (commas, float, scientific etc.) one or the other may
 *  be generated by any combination of each).
 */

import {typeOf, unfoldr} from 'fjl';
import {getErrorMsgByKey, ValidationResult} from "./Validator";
...
export const
    ...
    validate = (value, options) => {
        let result = false,
            ...
            isAString = classOfValue === 'String',
            isUnderStringMaxLength = isAString ?
                value.length < options.maxLength : true;
        ...
        else if (!isUnderStringMaxLength) {
            messages.push(getErrorMsgByKey('EXCEEDS_MAX_LENGTH', value, options));
        }
        else if (isAString) {
    ...
    validateComma = (value, options, messages = []) => {
        let out = 0,
            valueHasCommas = value.indexOf(',') > -1,
            replacedString;
        if (valueHasCommas) {
            if (options.allowCommas) {
                replacedString = value.replace(options.regexForComma, '');
    ...
    validateScientific = (value, options, messages = []) => {
        ...
        if (possibleScientific) {
            ...
            else if (!options.allowScientific && isValidScientificValue) {
...
NumberValidator.defaultOptions = {
    messageTemplates: {
        ...
        EXCEEDS_MAX_LENGTH: (value, options) =>
            `Passed in value exceeds \`maxLength\` of ${options.maxLength}` +
            `Value given: ${value}`
    },
    ...
    checkRange:  false,
    maxLength: (Number.MAX_SAFE_INTEGER + '').length,
